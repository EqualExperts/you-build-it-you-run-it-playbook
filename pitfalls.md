...
These are the pitfalls we've encountered when implementing [You Build It You Run It](https://app.gitbook.com/o/-LotYsHXVKys1Z4wI\_dX/s/-MXqoxcw7vw1rB7kpYNU/). Each pitfall drains confidence in the operating model, and ultimately puts it in jeopardy.

It's particularly important to guard against these pitfalls if your organisation previously migrated away from [Ops Run It](what-is-ops-run-it/). A predisposition to central operations teams may still exist in your organisation. If one or more pitfalls have a negative impact, your senior leadership may see a return to Ops Run It as a quick, cost efficient way to safeguard the reliability of digital services.
...
On-call standby costs are a key operating model cost, and a linear run cost creates a perception that You Build It You Run It is too expensive. If you also suffer from the [responsible but unaccountable pitfall](pitfalls.md#responsible-but-unaccountable), on-call funding is a line item in a strained opex budget and it shows linear growth.

You Build It You Run It has a [risk of high run costs](https://app.gitbook.com/o/-LotYsHXVKys1Z4wI\_dX/s/-MXqoxcw7vw1rB7kpYNU/), but it's not inevitable. Furthermore, run cost alone is a flawed comparison of You Build It You Run It and Ops Run It. Operating models are [different insurance policies for different business outcomes](practices/), and their costs are multi-faceted. The [high opportunity costs](introduction/#what-is-in-this-playbook) of Ops Run It are unacceptable for digital services.

To avoid this pitfall:

1. [Select availability targets on financial exposure](practices/), so each product team balances availability with engineering effort
2. [Select out of hours schedules on financial exposure](practices/), so product teams at scale do not produce linear run costs
...
1. [Make product team budget holders accountable for business outcomes](practices/), so product teams have sole accountability for their digital services
2. [Fund on-call costs from product team budgets](practices/), so product teams have to fund on-call themselves
...
* [Treat unplanned operational rework like planned work](practices/build.md#treat-unplanned-operational-rework-like-planned-work), so operational rework can be tracked and managed like any planned work
* [Re-architect digital services for adaptability](practices/build.md#architect-for-adaptability), so graceful degradation on failure is possible
* [Create a fully automated deployment pipeline](practices/build.md#be-prepared-to-deploy-at-any-time), so failed deployments are minimised and can be quickly reverted
* [Establish an automated telemetry toolchain](practices/build.md#automate-a-telemetry-toolchain), so dashboards and alerts are reliable and can be updated at any time
...
* [Pre-approve low risk, repeatable changes](practices/operational-enablers.md#pre-approve-low-risk-repeatable-changes) to accelerate a majority of deployments.
* [Automate change auditing](practices/operational-enablers.md#automate-change-auditing) for compliance with change management processes.
...
* [Integrate into incident management as is](practices/operational-enablers.md#integrate-into-incident-management-as-is), to ensure incident managers can be incident commanders for digital services as well as foundational systems.
...
* [Establish specialists as a service](practices/operational-enablers.md#establish-specialists-as-a-service), so repeatable, common tasks are automated as self-service functions, and specialists in a central team are freed up to offer ad hoc expertise on demand.
...
1. [Prepare for on-call from day one](practices/incident-response.md#prepare-for-on-call-from-day-one), so product team members are well equipped to handle out of hours production alerts.
2. [Re-architect digital services for adaptability](practices/build.md#architect-for-adaptability), so digital services don't require substantial human intervention, and are fast to fix on failure
3. [Ensure fair remuneration for on-call developers](practices/governance.md#ensure-fair-remuneration-for-on-call-developers), so product team members feel compensated for the disruption to their personal lives.
4. [Craft a sustainable on-call schedule](practices/incident-response.md#craft-a-sustainable-on-call-schedule), so no one product team member spends too much time on-call out of hours.
